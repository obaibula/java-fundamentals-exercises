...
public class CrazyOptionals {
    ...
    public static double calculateTotalCreditBalance(List<CreditAccount> accounts) {
        return accounts.stream()
                .flatMapToDouble(getCreditAccountDoubleStreamFunction())
                .sum();
    }

    private static Function<CreditAccount, DoubleStream> getCreditAccountDoubleStreamFunction() {
        return a -> DoubleStream.of(a.getCreditBalance()
                .orElseThrow()
                .doubleValue());
    }
}
